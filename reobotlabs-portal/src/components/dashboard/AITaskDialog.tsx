...
export function AITaskDialog({ clients, projects, open, onOpenChange }: AITaskDialogProps) {
  ...
  function handleAnalyze() {
    ...
    startAnalyzing(async () => {
      ...
      if (!result.tasks || result.tasks.length === 0) {
        toast.warning('Nenhuma tarefa identificada no texto. Tente com um texto mais detalhado.')
        return
      }
      ...
      <DialogContent className="dashboard-surface max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            {step === 'input' ? 'IA · Criar Tarefas' : `${reviewTasks.length} tarefas encontradas`}
          </DialogTitle>
          {step === 'review' && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Revise e edite antes de criar • {projectName}
            </p>
          )}
        </DialogHeader>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto flex-1 pr-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500/60 transition"
                >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500/60 transition disabled:opacity-50"
                >
                  <option value="">
            ...
            <div className="flex-1">
              ...
              <textarea
                ...
                placeholder={
                  sourceType === 'ata'
                    ? 'Cole a ata da reunião aqui...\n\nEx: Discutimos a criação do módulo de login. João ficou responsável por implementar o backend até sexta. Maria vai criar as telas no Figma esta semana...'
                    : sourceType === 'briefing'
            ...
            <div className="flex justify-end gap-2 shrink-0 pt-2 border-t border-border">
              ...
              <Button
                ...
                disabled={isAnalyzing || !text.trim() || !selectedProjectId}
